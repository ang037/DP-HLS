                            ...
                            map<string, string> &alignments)
{
    ...
    for (int i = 0; i < SOL_MAX_QUERY_LENGTH; i++)
    {
        ...
        initial_col[i][0] = -numeric_limits<double>::infinity();
        initial_col[i][1] = init_penal;
        initial_col[i][2] = -numeric_limits<double>::infinity(); // This can be whatever, since won't be accessed
    }
    init_penal = penalties.log_lambda;
    for (int j = 0; j < SOL_MAX_REFERENCE_LENGTH; j++)
    {
        init_penal += penalties.log_1_m_mu;
        initial_row[j][0] = -numeric_limits<double>::infinity(); // This can be whatever, since won't be accessed
        initial_row[j][1] = init_penal;
        initial_row[j][2] = -numeric_limits<double>::infinity();
    }
    ...
    {
        double del_up, del_diag;
        double ins_left, ins_diag;
        ...
        for (int j = 0; j < reference.length(); j++)
        {
            ...
            double del_write, ins_write, main_write;
            double ins_open, ins_extend, del_open, del_extend;
            ...
            ins_write = penalties.transition[4][HostUtils::Sequence::base_to_num(reference[j])] + (ins_open_b ? ins_open : ins_extend);
            del_write = penalties.transition[HostUtils::Sequence::base_to_num(query[i])][4] + (del_open_b ? del_open : del_extend);
            ...
            main_write = penalties.transition[
                HostUtils::Sequence::base_to_num(query[i])
            ][
                HostUtils::Sequence::base_to_num(reference[j])
            ] + main_max;
