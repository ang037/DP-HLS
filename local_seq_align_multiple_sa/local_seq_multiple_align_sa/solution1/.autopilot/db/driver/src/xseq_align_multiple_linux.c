...
typedef struct {
    ...
    XSeq_align_multiple_uio_map maps[ MAX_UIO_MAPS ];
} XSeq_align_multiple_uio_info;

/***************** Variable Definitions **************************************/
static XSeq_align_multiple_uio_info uio_info;
...
static int uio_info_read_name(XSeq_align_multiple_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XSeq_align_multiple_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XSeq_align_multiple_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XSeq_align_multiple_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XSeq_align_multiple_Initialize(XSeq_align_multiple *InstancePtr, const char* InstanceName) {
	XSeq_align_multiple_uio_info *InfoPtr = &uio_info;
